...
# Crear archivo temporal con lista de archivos a copiar
...
Get-ChildItem -Recurse -File | Where-Object {
    ...
    -not $exclude
} | ForEach-Object { $_.FullName.Replace((Get-Location).Path + "\", "") } | Out-File $tempFileList

# Copiar archivos usando SCP (método simple)
Write-Host "  Copiando archivos principales..." -ForegroundColor Gray

# Copiar archivos principales primero
$mainFiles = @("package.json", "package-lock.json", "ecosystem.config.js", "tailwind.config.js", "schema.prisma")
...
Write-Host ""
$url = "http://${VPS_HOST}:3005"
Write-Host "  3. Accede a: $url" -ForegroundColor Cyan
Write-Host ""
