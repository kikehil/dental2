...
// Eliminar doctor (soft delete - desactivar)
...
// Eliminar doctor permanentemente (hard delete)
const destroyPermanent = async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    
    // Verificar si el doctor tiene relaciones que impidan la eliminación
    const doctor = await prisma.doctor.findUnique({
      where: { id },
      include: {
        citas: { take: 1 },
        consultas: { take: 1 },
        ventas: { take: 1 },
        tratamientosPlazo: { take: 1 },
        serviciosLaboratorio: { take: 1 },
        usosMateriales: { take: 1 },
        prestamos: { take: 1 },
        usuario: true,
      },
    });
    ...
    // Eliminar permanentemente (las relaciones con onDelete: Cascade se eliminarán automáticamente)
    ...
    console.error('Error al eliminar doctor permanentemente:', error);
    // Si hay error por relaciones, redirigir con mensaje
    res.redirect('/doctores');
  }
};
